...
interface PurchaseApprovalModalProps {
  ...
  totalAmount: number;
  budgetLeft?: number; // 선택적으로 받을 수 있는 남은 예산
}
...
export default function PurchaseApprovalModal({
  ...
  const isOverBudget =
  typeof budgetLeft === 'number' &&
  calculatedTotal > budgetLeft;  // 예산 초과 여부

  useEffect(() => {
    if (totalAmount !== calculatedTotal) {
      setAmountError(
        `총 금액이 ${totalAmount.toLocaleString()}원과 다릅니다.`
      );
    } else {
      setAmountError(null);
    }
  ...
  return (
    <BaseFormModal
      ...
      confirmDisabled={!isValid || isOverBudget || totalAmount !== calculatedTotal}
