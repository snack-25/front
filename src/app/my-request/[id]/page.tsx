    ...
    <div className='w-full min-h-screen bg-[#FBF8F4] px-4 lg:px-16 pt-10 pb-10'>
      ...
      <div className='flex flex-col lg:hidden gap-6'>
        ...
        <div>
          ...
          <label className='block mt-4 text-lg font-semibold'>승인일</label>
          <input
            readOnly
            value={order.approvedAt || '-'}
            className='w-full border px-4 py-3 rounded-md text-gray-500'
          />
          <label className='block mt-4 text-lg font-semibold'>승인자</label>
          <input
            readOnly
            value={order.approver || '-'}
            className='w-full border px-4 py-3 rounded-md text-gray-500'
          />
          <label className='block mt-4 text-lg font-semibold'>상태</label>
          <input
            readOnly
            value={getStatusLabel(order.status)}
            className='w-full border px-4 py-3 rounded-md text-gray-500'
          />
          <label className='block mt-4 text-lg font-semibold'>응답 메시지</label>
          <textarea
            readOnly
            value={order.resultMessage || ''}
